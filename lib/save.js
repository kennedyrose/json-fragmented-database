...
function createFileContents(opt){
	return new Promise((resolve, reject) => {
		...
		opt.info.lastFileTotal = opt.files[fileCursor].length
...
function saveFiles(opt){
	return new Promise((resolve, reject) => {
		const promises = [new Promise((resolve, reject) => {
			fs.outputJson(`${opt.dir}/info.json`, opt.info, { spaces: '\t' })
				.then(resolve)
				.catch(reject)
		})]
		for(let i in opt.files){
			promises.push(new Promise((resolve, reject) => {
				fs.outputJson(`${opt.dir}/${i}.json`, opt.files[i], { spaces: '\t' })
					.then(resolve)
					.catch(reject)
			}))
		}
